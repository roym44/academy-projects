...
public class GreedyAlgorithmTester {
    ...
    public static void mstTest(){
        ...
        List<Graph.Edge> sol1 = Arrays.asList(s1,s3,s5,s6); // false
        List<Graph.Edge> sol2 = Arrays.asList(s1,s3,s4,s2); // false
        List<Graph.Edge> sol3 = Arrays.asList(s1,s3,s4,s6); // true
        List<Graph.Edge> sol4 = Arrays.asList(s1,s3,s5,s2); // false
        if (lst.equals(sol3))
